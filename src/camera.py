...
class Camera:
    def __init__(self, h, w, position, target, up=None, fovy=None, fy=None, rotation=None):
        self.znear = 0.01
        self.zfar = 100
        self.h = h
        self.w = w
        ...
        if fovy is not None:
            ...
            # Calculate vertical field of view (fovy) from focal length fy
            # The formula is: fy = h / (2 * tan(fovy / 2))
            # Rearranged: fovy = 2 * atan(h / (2 * fy))
        ...
        # self.up = np.array(up) if up is not None else np.array([0.0, -1.0, 0.0])
    ...
    @classmethod
    def from_pos_rot(cls, h, w, position, rotation, fy=None):
        """
        Creates a Camera instance from a 4x4 camera-to-world transformation matrix.
        """
        fovy = None
        if fy is not None:
            # Calculate vertical field of view (fovy) from focal length fy
        ...
        # In a standard right-handed coordinate system (like OpenGL), the camera
        # looks down its negative Z axis. The 'up' vector is the camera's local Y axis.
        forward = -rotation[:, 2]
        target = position + forward

        # up = rotation[:, 1]
        
        # Create a new camera instance using the calculated parameters.
        return cls(h, w, position=position, target=target, fovy=fovy, rotation=rotation)
    ...
    def get_view_matrix(self):
        # pyglm's lookAt function expects glm.vec3 objects, not numpy arrays.
        # We convert the numpy arrays to the correct type before calling the function.
        pos_glm = glm.vec3(*self.position)
        target_glm = glm.vec3(*self.target)
        up_glm = glm.vec3(*self.up)
        return np.array(glm.lookAt(pos_glm, target_glm, up_glm))
